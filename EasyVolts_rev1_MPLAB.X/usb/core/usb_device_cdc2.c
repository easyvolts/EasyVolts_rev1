...
/** V A R I A B L E S ********************************************************/
volatile unsigned char cdc_data_tx2[CDC_DATA_IN_EP_SIZE] IN_DATA_BUFFER_ADDRESS_TAG;
volatile unsigned char cdc_data_rx2[CDC_DATA_OUT_EP_SIZE] OUT_DATA_BUFFER_ADDRESS_TAG;

typedef union
{
    LINE_CODING lineCoding2;
    CDC_NOTICE cdcNotice2;
} CONTROL_BUFFER;
...
CDC_NOTICE cdc_notice2;
    ...
    SERIAL_STATE_NOTIFICATION SerialStatePacket2;
#endif
...
CONTROL_SIGNAL_BITMAP control_signal_bitmap2;
uint32_t BaudRateGen2;			// BRG value calculated from baud rate

#if defined(USB_CDC_SUPPORT_DSR_REPORTING)
    BM_SERIAL_STATE SerialStateBitmap2;
    BM_SERIAL_STATE OldSerialStateBitmap2;
    USB_HANDLE CDCNotificationInHandle2;
#endif
...
uint8_t dummy_encapsulated_cmd_response2[dummy_length];
...
void USBCDCSetLineCoding2(void);
...
void USBCheckCDCRequest2(void)
{
     ...
     */
    if((SetupPkt.bIntfID != CDC_COMM_INTF_ID2)&&
    ...
    switch(SetupPkt.bRequest)
    {
        ...
        case SEND_ENCAPSULATED_COMMAND:
         //send the packet
            inPipes[0].pSrc.bRam = (uint8_t*)&dummy_encapsulated_cmd_response2;
        ...
        case SET_CONTROL_LINE_STATE:
            ...
            //------------------------------------------------------------------            
                ...
                if(control_signal_bitmap2.DTE_PRESENT == 1)
                {
                    UART_DTR2 = USB_CDC_DTR_ACTIVE_LEVEL;
                }
                else
                {
                    UART_DTR2 = (USB_CDC_DTR_ACTIVE_LEVEL ^ 1);
                }        
            #endif
            inPipes[0].info.bits.busy = 1;
            break;
        ...
        case SEND_BREAK:                        // Optional
			...
			{
				UART_Tx2 = 0;       // Prepare to drive TX low (for break signaling)
				UART_TRISTx2 = 0;   // Make sure TX pin configured as an output
				UART_ENABLE2 = 0;   // Turn off USART (to relinquish TX pin control)
			}
			else if (SetupPkt.wValue == 0x0000) //0x0000 means stop sending indefinite break 
			{
    			UART_ENABLE2 = 1;   // turn on USART
				UART_TRISTx2 = 1;   // Make TX pin an input
			}
			else
			{
...
void CDCInitEP2(void)
{
    ...
    #if defined(USB_CDC_SUPPORT_DSR_REPORTING)
      	CDCNotificationInHandle2 = NULL;
        mInitDTSPin2();  //Configure DTS as a digital input
      	SerialStateBitmap2.byte = 0x00;
      	OldSerialStateBitmap2.byte = !SerialStateBitmap2.byte;    //To force firmware to send an initial serial state packet to the host.
        //Prepare a SerialState notification element packet (contains info like DSR state)
        SerialStatePacket2.bmRequestType = 0xA1; //Always 0xA1 for this type of packet.
        SerialStatePacket2.bNotification = SERIAL_STATE;
        SerialStatePacket2.wValue = 0x0000;  //Always 0x0000 for this type of packet
        SerialStatePacket2.wIndex = CDC_COMM_INTF_ID;  //Interface number  
        SerialStatePacket2.SerialState.byte = 0x00;
        SerialStatePacket2.Reserved = 0x00;
        SerialStatePacket2.wLength = 0x02;   //Always 2 bytes for this type of packet    
        CDCNotificationHandler2();
  	#endif
...
/**************************************************************************
  ...
  **************************************************************************/
#if defined(USB_CDC_SUPPORT_DSR_REPORTING)
void CDCNotificationHandler2(void)
{
    ...
    if(UART_DTS2 == USB_CDC_DSR_ACTIVE_LEVEL) //UART_DTS must be defined to be an I/O pin in the hardware profile to use the DTS feature (ex: "PORTXbits.RXY")
    {
        SerialStateBitmap2.bits.DSR = 1;
    }  
    else
    {
        SerialStateBitmap2.bits.DSR = 0;
    }        
    
    //If the state has changed, and the endpoint is available, send a packet to
    //notify the hUSB host of the change.
    if((SerialStateBitmap2.byte != OldSerialStateBitmap2.byte) && (!USBHandleBusy(CDCNotificationInHandle2)))
    {
        //Copy the updated value into the USB packet buffer to send.
        SerialStatePacket2.SerialState.byte = SerialStateBitmap2.byte;
        ...
        CDCNotificationInHandle2 = USBTransferOnePacket(CDC_COMM_EP2, IN_TO_HOST, (uint8_t*)&SerialStatePacket2, sizeof(SERIAL_STATE_NOTIFICATION));
        
        //Save the old value, so we can detect changes later.
        OldSerialStateBitmap2.byte = SerialStateBitmap2.byte;
    }    
}//void CDCNotificationHandler2(void)    
#else
    #define CDCNotificationHandler2() {}
#endif
...
/**********************************************************************************
  ...
  **********************************************************************************/
bool USBCDCEventHandler2(USB_EVENT event, void *pdata, uint16_t size)
{
...
void CDCTxService2(void)
{
    ...
    else if(cdc_trf_state2 == CDC_TX_BUSY)
    {
        ...
        if(cdc_tx_len2 == 0)
        {
            if(byte_to_send == CDC_DATA_IN_EP_SIZE2)
